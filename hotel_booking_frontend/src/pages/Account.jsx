...
const Account = ({ contactSectionRef, aboutSectionRef }) => {
  ...
  const fetchPendingPayments = async (userId) => {
    try {
      ...
      const response = await fetch(`http://localhost:9090/payments/pending?userId=${userId}`, {
  ...
  useEffect(() => {
    const fetchAccountDetails = async () => {
      ...
      try {
        ...
        // Fetch pending payments after user is loaded and has id
        if (userRes.data.id) {
          fetchPendingPayments(userRes.data.id);
        }
      } catch (err) {
        setUser(null);
  ...
  const fetchBookings = async () => {
    try {
      ...
      setUser(prevUser => ({ ...prevUser, bookings: res.data }));
    } catch (err) {
  ...
  const fetchPayments = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`${API_BASE}/payments/me`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setUser(prevUser => ({ ...prevUser, payments: res.data }));
    } catch (err) {
    ...
    <div className="min-h-screen bg-black py-10 px-4">
      ...
      <div className="max-w-3xl mx-auto bg-[#181f2a] rounded-2xl shadow-2xl p-8 border border-gray-800">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-extrabold text-[#00df9a] tracking-wide drop-shadow">My Account</h1>
          <button
