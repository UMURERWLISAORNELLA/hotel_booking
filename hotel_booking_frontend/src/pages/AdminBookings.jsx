                ...
                <tbody>
                  {bookings.map((b, index) => (
                    <tr key={b.bookingId} className="border-b border-gray-800">
                      <td className="px-4 py-2 text-[#00df9a]">{b.guestName || b.guest?.fullName || "-"}</td>
                      <td className="px-4 py-2 text-white">{b.hotelName || b.hotel?.name || "-"}</td>
                      <td className="px-4 py-2 text-white">{b.room || `${b.roomNumber || "-"} (${b.roomType || "-"})`}</td>
                      <td className="px-4 py-2 text-white">{b.checkInDate}</td>
                      <td className="px-4 py-2 text-white">{b.checkOutDate}</td>
                      <td className="px-4 py-2 text-white">
                        <span className={`px-2 py-1 rounded text-xs ${
                          b.bookingStatus === 'CONFIRMED' ? 'bg-green-600 text-white' :
                          b.bookingStatus === 'PENDING' ? 'bg-yellow-600 text-white' :
                          b.bookingStatus === 'CANCELLED' ? 'bg-red-600 text-white' :
        ...
        {showDelete && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center">
              ...
              <p className="mb-6">Are you sure you want to delete the booking for <span className="font-semibold">{bookingToDelete?.guestName || bookingToDelete?.guest?.fullName}</span> at <span className="font-semibold">{bookingToDelete?.hotelName || bookingToDelete?.hotel?.name}</span>?</p>
