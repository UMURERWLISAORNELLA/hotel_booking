...
const AdminHotels = ({ getUser }) => {
  ...
  const fetchHotels = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_BASE}/hotels/admin?page=${page}&size=${size}`, {
  ...
  const handleSave = async () => {
    // Validation
    if (!formHotel.name || !formHotel.region) {
      alert("Please fill in both name and region fields.");
      return;
    }
    ...
    try {
      if (editingHotel) {
        // Update
        const response = await axios.put(`${API_BASE}/hotels/${editingHotel.hotelId}`, formHotel, {
          headers: { Authorization: `Bearer ${token}` },
        });
        
        if (response.data && typeof response.data === 'string' && response.data.includes('Invalid region')) {
          alert(response.data);
          return;
        }
      } else {
        // Create
        const response = await axios.post(`${API_BASE}/hotels`, formHotel, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        {showDelete && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center">
              ...
              <p className="mb-6">Are you sure you want to delete <span className="font-semibold">{hotelToDelete?.name}</span> from {hotelToDelete?.region}?</p>
